...
$\textit{NPoints}_\mathrm{ST} = \textit{NPoints}_\mathrm{spatial} \times (\texttt{ot}+1)$.  The
on-disk byte order is: \emph{all spatial coordinates of all space-time points first}, followed by
\emph{all temporal coordinates as a contiguous trailing block}.  The temporal coordinates are
\emph{not} interleaved with the spatial ones.

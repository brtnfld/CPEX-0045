...
\begin{table}[htbp]
\centering
\caption{Lagrange functional spaces per element and interpolation type.  The base type and order
  $p$ are specified for solution interpolation; the full element type is used to classify element
  interpolation.}
\label{tab:spaces}
\small
...
\begin{sidsnode}{FlowSolution\_t :=}
List( Descriptor\_t Descriptor1 ... DescriptorN );            \textrm{(o)}\\
...
\midrule
\texttt{ierr = cg\_element\_lagrange\_interpolation\_size(ElementType\_t t, cgsize\_t *sz)} & r{-}{-} \\
\texttt{ierr = cg\_solution\_lagrange\_interpolation\_size(ElementType\_t t, int os, int ot, cgsize\_t *sz)} & r{-}{-} \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{} l l l @{}}
\toprule
\textbf{Child name} & \textbf{Properties} & \textbf{Comments} \\
\midrule
\texttt{LagrangeControlPoints} &
  type: \texttt{DataArray\_t}, dtype: \texttt{R8}, dims: [2] &
  table [\textit{Dimension}][\textit{NumberOfPoints}];
...
\texttt{InterpolationType} &
  dtype: \texttt{InterpolationType\_t} &
  one of \texttt{ParametricLagrange}, \texttt{Parametric\-Monomials\-Pascal},
  \texttt{CartesianMonomialsPascal}, \texttt{IsoParametric} \\
\addlinespace
\texttt{LagrangeControlPoints} &
  type: \texttt{DataArray\_t}, dtype: \texttt{R8}, dims: [2], cardinality: 0:1 &
  table [\textit{Dimension}][\textit{NumberOfPoints}]; dimension = element
  dimension, may be incremented by~1 for space-time \\
\bottomrule
\end{tabular}

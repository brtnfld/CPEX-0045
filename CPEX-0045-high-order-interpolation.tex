...
\noindent\textbf{Implementation note:} Although the SIDS prose above lists
\texttt{SpatialOrder} and \texttt{TemporalOrder} as scalar fields of \texttt{FlowSolution\_t},
the normative on-disk encoding stores them as a single \texttt{IndexArray\_t} child node named
\texttt{InterpolationOrders} of dimension~2, with values \texttt{[spatialOrder, temporalOrder]}.
This is the form described in Section~\ref{sec:filemap} and implemented by

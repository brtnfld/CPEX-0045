...
| m4 | `.tex:596` | Completeness | Minor | Fekete distributions are named as one of the two acceptable choices for simplices at `p>2`, but `Fekete` is not among the four enumeration values, so a writer that uses Fekete nodes — which this sentence says is required — has no value to record and must fall back to `ControlPointDistributionUserDefined`. The document does not say so. | Either add `Fekete` to the enumeration, or state that Fekete nodes are recorded as `UserDefined`. Since the coordinates are authoritative (`:599-608`), the second is sufficient and cheaper. |
...
- **The precedence rule** — stored coordinates authoritative, distribution name advisory, reader
  ...
  applied across `ControlPointDistribution` and `CharacteristicLength`, and correctly
